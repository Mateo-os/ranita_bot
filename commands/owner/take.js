...
async function take(player, message, args){
    ...
    if (playerCard){
        if (playerCard.Cromo.cantidad == 1){
            await playerCard.Cromo.destroy();
        }else{
            await playerCard.Cromo.decrement('cantidad');
        }
        playerCard.save();
    } else {
        return [`${member.nombre} no posee la carta ${card.nombre}`];
    }
    return [`Se le ha quitado ${card.nombre} a ${member.nombre}`];
}
